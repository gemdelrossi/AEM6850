...
```{r scatter and line plots, results = 'hide'}
...
What happens when you want to plot outside the box?? Have to REALLY specify it. Set `par(xpd=TRUE)` to put it
outside the plotting box. When working with parameters, have to turn it **on/off** 

**Add text....**

1. find a way to get the GDP each decade automatically

Command `which` gives you TRUE/FALSE. Applying `idx <- which(data$time %in% seq(1950,2010,10))` creates an index
for each year on those select years. 
...
The `text(data$time[idx]+4,data$GDPC1[idx]-400, round(data$GDPC1[idx]), col="blue")` command puts labels.
Adding numbers when specifying the GDP and time vars puts them to the side of the points specified. 
...
Make sure to set `type="windows"` for png because working on a Windows computer. 
...
`beside = T/F` means that if false, the columns of height are portrayed as stacked bars, 
...
For colors: need n-1 colors from the breaks
  `colors <- colorRampPalette(brewer.pal(9, "Reds"))(length(breaks)-1)`
If need more colors, can interpolate using `colorRampPalette`. 
...
Provides a bit more flexibility, quicker than using the above command (in terms of loading/rendering). 
...
4. Select files for 1997-2017 only for simplicity
